...
function EditNotes({ route, navigation }) {
  const { note, refreshNotes } = route.params; // Pass the refreshNotes callback function
  const [header, setHeader] = useState(note.header);
  const [comment, setComment] = useState(note.comment);
  const [date, setDate] = useState(note.date);
  ...
  //   const isNoteUpdated = await editDailyNote(token, userId, updatedNote);
  //   if (isNoteUpdated) {
  //     refreshNotes(); // Trigger the refresh of the note list
  //     navigation.navigate('notelist', { note: updatedNote });
  //   } else {
  //     console.error('Failed to update note.');
  //   }
  // }
  ...
  const saveNote = async () => {
    ...
    if (isNoteUpdated) {
      ...
      navigation.navigate('notedetails', { note: updatedNote }); // Navigate to notedetails with updated note data
   refreshNotes(); // Trigger the refresh of the note list
    } else {
      console.error('Failed to update note.');
    }
